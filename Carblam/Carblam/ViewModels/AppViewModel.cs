...
namespace Carblam.ViewModels
{
    ...
    public class AppViewModel : BindableObject
    {
        ...
        readonly CarblamSvcApi _api = new CarblamSvcApi("http://172.16.100.47:8181/mysvc", WebSvcMode.Xml);

        internal CarblamSvcApi Api { get { return _api; } }
        ...
        readonly Thread _locationReporingThread;
        ...
        public AppViewModel()
        {
            ...
            _locationReporingThread = new Thread(this.LocationReportingProc) { IsBackground = true };
            _locationReporingThread.Start();
        }
        ...
        private async void OnLogin()
        {
            ...
            _root.OrdersListPage.Refresh();
            _root.DriverPage.Setup();
        }
        ...
        ManualResetEvent _locationReportingEnableEvent = new ManualResetEvent(false);

        private void LocationReportingProc()
        {
            Location prevLoc = null;
            for(; ;)
            {
                _locationReportingEnableEvent.WaitOne();

                var loc = RouteHelper.FindCurrentLocation().Result;
                if (loc != null && (prevLoc == null || Location.CalculateDistance(loc,prevLoc, DistanceUnits.Kilometers) > 0.05))
                {
                    try
                    {
                        _api.UpdateLocation(new LocationType() { Latitude = loc.Latitude, Longitude = loc.Longitude }).Wait();
                    }
                    catch (WebApiException ex)
                    {
                        System.Diagnostics.Debug.Print(ex.ToString());
        ...
        {
            _locationReportingEnableEvent.Set();
        }

        public void StopLocationReporting()
        {
            _locationReportingEnableEvent.Reset();
        }
    }
